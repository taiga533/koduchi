...
/**
 * CodeMirror の補完へ渡す表を組み立てる（ADR 0007 の結果）。
 *
 * `@codemirror/lang-sql` は `{ テーブル名: 列名の並び }` を受け取る。スキーマを
 * またいで同じ表名があるため、`スキーマ名.表名` の形でも引けるようにする。
 *
 * 呼び出し側で `useMemo` に包むこと。
 *
 * @param schemas 取得済みのスキーマ
 * @param columns スキーマ名ごとの列情報
 */
export function buildCompletionSchema(
  ...
  for (const schema of schemas) {
    const 列 = new Map<string, string[]>()

    for (const column of columns[schema.name] ?? []) {
      const names = 列.get(column.objectName) ?? []
      names.push(column.name)
      列.set(column.objectName, names)
    }

    for (const object of schema.objects) {
      const names = 列.get(object.name) ?? []
      table[`${schema.name}.${object.name}`] = names

      // 修飾なしの名前は、先に現れたスキーマのものを優先する。
      if (!(object.name in table)) {
        table[object.name] = names
      }
    }
  }
