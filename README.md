...
| スキーマ            | 内容                                                                                                                                                                                                                                                                                                                                  |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `KODUCHI`           | `events`（50,000 行）・`orders`・`order_items`・`refunds`・`users`・`user_traits`、ビュー `session_rollup`、関数 `order_total`、プロシージャ `say_hello`（`DBMS_OUTPUT` の確認用）、値の型を網羅した `value_kinds`、および索引・トリガー・シノニム・型（[ADR 0014](adr/0014-スキーマツリーのオブジェクト種別拡張.md) の種別の確認用） |
