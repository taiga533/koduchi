...
**キーバインドの置き場所**: エディタの中でしか意味を持たない `⌘⏎` / `⇧⌘⏎` / `⌥⌘⏎` / `⌘.` と検索の `⌘F` / `⌘G` / `⇧⌘G` / `⌥⌘F` と整形の `⇧⌥F`（ADR 0024）は CodeMirror の keymap に、それ以外（`⌘E` / `⇧⌘E` / `⌥⌘S` / `⌥⌘C` / `⌥⌘R` / `⌘S` / `⇧⌘S` / `⌘O` / `⌘T` / `⌘W` / `⌃⌘N` / `⌘K` / `⇧⌘F`）は `App.tsx` の `keydown` に置く。後者は `event.defaultPrevented` を見て、エディタが既に処理したものを二重に扱わない。修飾の重なる `⌘S` / `⇧⌘S` / `⌥⌘S` は、絞りの強い枝から先に見る。結果テーブルの中でしか意味を持たない `⌘C` / `⇧⌘C` / `⌘A` は `ResultTable` の `keydown` に置き、スキーマツリーの中でしか意味を持たない `↑` / `↓` / `→` / `←` / `⌥⏎` / `⌘C` / `⌘D` は `SchemaTree` の `keydown` に置く（ADR 0020・0022）。タブ帯の中でしか意味を持たない `⌥←` / `⌥→`（タブの並べ替え）は `TabBar` の `keydown` に置く（ADR 0023）。`⌘I`（Ask AI）は**割り当てない**（将来のための予約）。`⌘K` はコマンドパレット（ADR 0018）、`⇧⌘F` はオブジェクトのソース検索（ADR 0021）に割り当て済みである。`⌘D`（定義タブを開く）はツリーの中だけで効く（ADR 0022）。`⇧⌘F` は CodeMirror の `⌘F` とは修飾が違うため食い合わない。`⇧⌥F`（SQL の整形、ADR 0024）だけは keymap へ**キーの名前で登録できない**。macOS では `⌥` を伴う打鍵で文字そのものが変わる（US でも JIS でも `Ï`）ため、keymap の `any` から `event.code === 'KeyF'` で拾う（`formatting.ts` の `isFormatShortcut`）。

**変換中の打鍵（IME）**（ADR 0025）: 日本語の変換確定の `⏎` は「決定」ではない。**修飾キーの付かない打鍵（`⏎` / `esc` / `↑` / `↓`）を扱うハンドラは、先頭で `src/input/ime.ts` の `isComposingKey(event)` を見て、真なら何もせずに戻る。**`<form>` には `onKeyDown={blockComposingSubmit}` を付ける（中の欄が日本語を打つ欄かで判断しない）。**WKWebView は変換中の `⏎` で暗黙の送信をしてしまう**ことを実測してある。**変換を確定する `⏎` は `isComposing` が偽・`keyCode` が 229 で届く**（本物の Kotoeri で測り直した。合成した変換状態での初回の測定は誤っていた。ADR 0025 の「1-2」）。**`isComposing` と `keyCode === 229` の両方を見る。**片方だけでは確定の打鍵を取りこぼす。**確定直後の打鍵を時間や回数で捨てる手当ては入れない**（「日本語を打った直後だけ `⏎` が効かない」という別の不具合になる）。`⌘` や `⌥` を必須にする打鍵（`App.tsx` の `keydown`・`SchemaTree` の `⌥⏎`・`TabBar` の `⌥←` など）には要らない。**絞り込みの入力欄では変換中の入力を止めない**（手元の配列を篩に掛けるだけで、IPC も問い合わせも起きない。ソース検索はそもそも打鍵のたびには投げない。ADR 0021）。**CodeMirror の本文には手当てを足さない。**`@codemirror/view` の `ignoreDuringComposition` が変換中の打鍵を既に捨てており、WebKit で `compositionend` の後に `⏎` が届く癖まで見ている。**`⌘F` の検索パネルだけが対象外**（`contentDOM` の外にあるため）で、変換中の `esc` でパネルごと閉じてしまう。`search.tsx` の捕捉相の関所へ `keyup` に加えて `keydown` も通して塞ぐ。**`preventDefault()` は使わない**（IME の確定そのものへ影響しかねない）。詳細と測り方の落とし穴は ADR 0025 の「1-3」。**入力欄や `<form>` を足したら ADR 0025 の監査の表に 1 行を足す。**
