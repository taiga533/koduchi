...
**変換中の打鍵（IME）**（ADR 0025）: 日本語の変換確定の `⏎` は「決定」ではない。**修飾キーの付かない打鍵（`⏎` / `esc` / `↑` / `↓`）を扱うハンドラは、先頭で `src/input/ime.ts` の `isComposingKey(event)` を見て、真なら何もせずに戻る。**`<form>` には `onKeyDown={blockComposingSubmit}` を付ける（中の欄が日本語を打つ欄かで判断しない）。**WKWebView は変換中の `⏎` で暗黙の送信をしてしまう**ことを実測してある。`key` は `Enter` のまま・`keyCode` は 13 で届き（Chromium と違い 229 にならない）、**見分ける手立ては `isComposing` だけ**である。`keyCode === 229` を併せて見ない。**確定直後の打鍵を時間や回数で捨てる手当ては入れない**（「日本語を打った直後だけ `⏎` が効かない」という別の不具合になる）。`⌘` や `⌥` を必須にする打鍵（`App.tsx` の `keydown`・`SchemaTree` の `⌥⏎`・`TabBar` の `⌥←` など）には要らない。**絞り込みの入力欄では変換中の入力を止めない**（手元の配列を篩に掛けるだけで、IPC も問い合わせも起きない。ソース検索はそもそも打鍵のたびには投げない。ADR 0021）。CodeMirror の検索パネルだけは `src/components/editor/search.tsx` が捕捉相で `keyup` を止めており、そこは `@codemirror/search` の中へ手が届かないための例外である。**入力欄や `<form>` を足したら ADR 0025 の監査の表に 1 行を足す。**
